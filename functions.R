...
mu10t <- function(args) {
  ...
  if(args$epistasis) {
    mu10 <- mu00t(args) + ifelse(abs(args$R1) > mu00t(args), sign(args$R1)*mu00t(args), args$R1)
  }
  mu10
}
...
mu01t <- function(args) {
  ...
  if(args$epistasis) {
    mu01 <- mu00t(args) + ifelse(abs(args$R2) > mu00t(args), sign(args$R2)*mu00t(args), args$R2)
  }
  mu01
}
